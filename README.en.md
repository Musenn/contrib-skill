...
- **Causal, high-density contributions**: keeps the summary to one or two sentences, then writes three or four numbered items as `technical theme: problem/constraint → approach and mechanism → evidence-backed result`; metrics appear only when a saved source supports them
