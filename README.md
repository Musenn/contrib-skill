...
- **简介与贡献比例可控**：项目简介压缩为 1—2 句需求背景与系统定位，技术方案、关键机制及有证据结果集中写入 3—4 条编号贡献；每条至少覆盖“负责对象、技术方案、关键机制、解决问题、实测结果”中的三项
...
> 📂 完整输出示例见 [docs/example-output/](docs/example-output/)（一个模拟电商仓库的真实运行结果，未做手工修改）。项目简介保持两句，编号集中展开框架、状态驱动、缓存与数据访问机制；MVC/DDD/Repository/Strategy 等词汇只在证据命中时出现。该示例未提供压测报告，因此不会生成量化性能指标。数据如何写入简历见 [本地压测数据示例](docs/benchmark-resume-example.md)。
